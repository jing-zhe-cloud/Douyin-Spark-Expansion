...
- 网页顶部实时状态栏，实时展示程序正在执行的动作（启动浏览器、打开抖音、检查好友、发送消息等）
- 状态每 3 秒自动刷新，配合日志可快速定位卡在哪一步

### 7. 反检测与连接稳定性

- 无头浏览器附加反自动化检测参数，并伪装真实 User-Agent，降低被抖音识别并断开连接的概率
- 打开页面失败时自动重试 3 次，缓解 `ERR_CONNECTION_CLOSED` 等网络抖动问题
